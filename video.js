<!--
  Video Gallery — Trends Saj Parbon (Webflow-ready)
  Usage:
  1) Add a Webflow Embed element and paste this entire block.
  2) Optionally move the <style> to Site Settings → Custom Code → Head.
  3) If you already have a site header, remove the simple top bar here.
-->
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    html,body{margin:0;padding:0;background:var(--bg);font-family:ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, Helvetica, Arial, "Apple Color Emoji","Segoe UI Emoji";color:#eaf2f8}
    img{max-width:100%;display:block}
    .container{max-width:var(--container);margin:0 auto;padding:0 16px}
    .topbar{position:sticky;top:0;z-index:10;background:linear-gradient(180deg,#0f2234,#0a1724);border-bottom:1px solid rgba(255,255,255,.06)}
    ...
    .heading h1{font-size:28px;line-height:1.2;color:#fff;margin:0}
    @media(min-width:768px){.heading h1{font-size:34px}}

    .lead{max-width:720px;margin:0 auto 24px;color:#b9cfde;text-align:center}

    .grid{display:grid;grid-template-columns:1fr;gap:24px;max-width:1040px;margin:0 auto;border:1px solid rgba(219,231,243,.6);border-top-width:2px;padding:24px;border-radius:var(--radius-2xl);background:linear-gradient(180deg,rgba(255,255,255,.02),rgba(255,255,255,.00))}
    ...
    .video{position:relative;width:100%;aspect-ratio:16/9;background:#0d2236}
    /* Fallback for older browsers without aspect-ratio */
    .video::before{content:"";display:block;padding-top:56.25%}
    .video iframe{position:absolute;inset:0;width:100%;height:100%;border:0}

    /* Clapperboard icon style */
...
<body>
  <!-- Simple Top Bar (replace/remove if you have your own header) -->
  <div class="topbar">
  ...
  <main class="page container">
    <header class="heading" aria-label="Video Gallery heading">
      <span class="icon" aria-hidden>
        <!-- Lucide Clapperboard (inline SVG) -->
  ...
  <script>
    // Set the document title (mirrors your useEffect)
